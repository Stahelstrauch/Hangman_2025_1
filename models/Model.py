...
class Model:
    ...
    def save_player_score(self, name, seconds):
        ...
        if self.database.cursor:
            try:
                sql = f'INSERT into {self.database.table_leaderboard} (name, word, letters, game_length, game_time) VALUES ("{name}", "{self.__new_word}", "{self.get_all_user_chars()}", "{seconds}", "{today}")'
